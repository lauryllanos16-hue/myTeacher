...
async function cargarResenas() {
  ...
  try {
    ...
    document.getElementById('promedioGlobal').textContent = datos.promedio || '0.0';
...
function renderComentarios() {
  ...
  if (slice.length === 0) {
    lista.innerHTML = '<p style="text-align:center;color:#888;">No hay reseñas todavía.</p>';
  ...
  btnVerMas.style.display = mostrandoHasta >= resenasData.length ? 'none' : 'inline-block';
}
...
async function enviarComentario() {
  ...
  if (calificacionSeleccionada === 0) { alert('Por favor selecciona una calificación.'); return; }
  if (!texto) { alert('Por favor escribe un comentario.'); return; }
  if (!reservaId) { alert('No se encontró la reserva.'); return; }
